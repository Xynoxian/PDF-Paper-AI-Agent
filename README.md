...
├── slm_tuner.py            # D4 SLM QLoRA fine-tuning + inference + caching
...
├── tests/
│   └── test_smoke.py       # Pytest smoke tests (D4)
├── configs/
│   ├── run_card.yaml       # AutoML winning config + metrics (D1)
│   └── tuning_card.yaml    # SLM tuning card (generated by slm_tuner.py)
├── models/
│   └── slm-tuned/          # QLoRA adapter weights (generated)
├── cache/
│   └── slm_responses/      # Disk cache for tuned SLM responses
└── Notebooks/
...
### D4 -- SLM Fine-Tuning & Final Integration

- `slm_tuner.py`: PEFT/QLoRA fine-tuning on a curated 15-pair Q/A dataset
  - 4-bit NF4 quantization via bitsandbytes (GPU) or fp32 fallback (CPU)
  - LoRA adapters (r=16, alpha=32) on `q_proj`/`v_proj` attention layers
  - Disk-based response cache to skip redundant inference
  - Generates `configs/tuning_card.yaml` with full reproducibility metadata
- `evaluate.py`: Extended with `hybrid_tuned` ablation mode (zero-shot vs tuned comparison)
- `graphrag_executor.py`: `use_tuned_slm` flag routes answer generation through the tuned model
- `tests/test_smoke.py`: 25+ pytest smoke tests covering imports, metrics, provenance, models

#### Quick Start (D4)

```bash
# 1. Generate the Q/A training dataset
python slm_tuner.py --generate-dataset

# 2. Fine-tune (requires ~2GB VRAM for TinyLlama, or runs on CPU)
python slm_tuner.py

# 3. Run full ablation including tuned model
python evaluate.py --csv eval_queries.csv --ablation
...
| `SLM_BASE_MODEL`| `TinyLlama/TinyLlama-1.1B-Chat-v1.0` | Base model for QLoRA tuning  |
| `SLM_EPOCHS`    | `3`                          | Number of fine-tuning epochs          |
| `SLM_LR`        | `2e-4`                       | SLM training learning rate            |
| `LORA_R`        | `16`                         | LoRA rank                             |
